...
SOURCE: VOOP reframe—keep it or sharpen it.
...
SOURCE: VOOP obstacle → the avoidance; add cost to raise stakes
...
function buildUserPrompt(
  ...
  return `VOOP DATA:
${JSON.stringify({ language_to_echo: voopData.language_to_echo, voop: voopData.voop }, null, 2)}

LANGUAGE TO ECHO (use these exact phrases):
${voopData.language_to_echo.map(l => `- "${l}"`).join('\n')}

STORY: "${story || 'No story provided.'}"

VALUES: ${valueNames} (IDs: ${values.map((v) => v.id).join(', ')})

Generate the Values Card. Create deeply personal definitions and commitments based on the VOOP data. Return valid JSON only.`;
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { values, story, voopData } = body;
    ...
    const stream = await client.messages.stream({
      ...
      messages: [
        {
          role: 'user',
          content: buildUserPrompt(values, story, voopData),
        },
      ],
    });
